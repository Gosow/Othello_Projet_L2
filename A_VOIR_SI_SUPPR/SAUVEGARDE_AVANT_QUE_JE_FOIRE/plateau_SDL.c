...
int lancement_jeu(int modeJeu, int type){
    ...
    if(modeJeu == ONLINE){
        if(type == SERVEUR){
            ...
            fprintf(stderr,"\n1 : '%s'\n",msg);
            if(strncmp("GO",msg,2)){
                fprintf(stderr,"LANCEMENT DE LA FUSEEEEE\n");
                strcpy(msg,"\nOK\n");
                fprintf(stderr,"\n2 : '%s'\n",msg);
                if(send_from_serv(msg)<0) fprintf(stderr,"Erreur\n");
                //send_from_serv(msg);
                fprintf(stderr,"ENVOYÉE\n");
            };
        }
        else if(type == CLIENT){
            fprintf(stderr,"RECUghfjhjgfghjf");
            init_client();
            envoyer_message("GO",msg);
            fprintf(stderr,"\nmsg : '%s'\n",msg);
            fprintf(stderr,"APPEL RECUghfjhjgfghjf");
            if(strncmp("OK",msg,2)){
                fprintf(stderr,"APPEL RECU");
            };
        }
    }
