...
char* envoyer_message(char* msg, char* buff){
	fprintf(stderr,"ACTE 1 DEPUIS CLIENT 1 : %s %s\n",buff,msg);
	sscanf(msg,"%s",buff);
	//sprintf(msg, "%s", buff);
	fprintf(stderr,"ACTE 2 DEPUIS CLIENT 1 : %s %s\n",buff,msg);
	send(to_server_socket, buff, strlen(buff), 0); //on augmente la taille de 4 pour l'entête
	// lecture de la réponse
	memset(buff, 0, sizeof(buff));
	recv(to_server_socket,buff,512,0);
	fprintf(stderr,"DEPUIS CLIENT : '%s' \n",buff);
	return buff;
